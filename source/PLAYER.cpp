...
void quickkill(struct player_struct *p)
{
	...
	sprite[p->i].extra = 0;
	sprite[p->i].cstat |= CSTAT_INVISIBLE;
...
long hitasprite(short i,short *hitsp)
{
	...
	if(hw >= 0 && (wall[hw].cstat&WSTAT_MASKED) && badguy(&sprite[i]) )
		return((1<<30));
...
short aim(spritetype *s,short aang)
{
	...
	for(k=0;k<4;k++)
	{
		...
		for(i=headspritestat[aimstats[k]];i >= 0;i=nextspritestat[i])
			if( sprite[i].xrepeat > 0 && sprite[i].extra >= 0 && (sprite[i].cstat&(CSTAT_ALLBLOCKING+CSTAT_INVISIBLE)) == CSTAT_ALLBLOCKING)
	...
	case KNEE:
			...
			if( FindDistance2D(sx-hitx,sy-hity) < 1024 )
				if( hitwall >= 0 && wall[hitwall].overpicnum != BIGFORCE )
					if( ( wall[hitwall].nextsector >= 0 && hitsect >= 0 &&
						...
						if( (wall[hitwall].cstat&WSTAT_MASKED) == 0)
						{
							...
							if(wall[hitwall].hitag == 0)
							{
								...
								sprite[k].y = hity;
								sprite[k].z = hitz;
								sprite[k].cstat |= (TRAND&CSTAT_XFLIP);
								ssp(k,CLIPMASK0);
		...
		if( ( klabs(sx-hitx)+klabs(sy-hity) ) < 1024 )
		{
			if(hitwall >= 0 || hitspr >= 0)
			{
				...
				{
					if( wall[hitwall].cstat&WSTAT_SWAPBOTTOMS )
						if(wall[hitwall].nextsector >= 0)
...
SKIPBULLETHOLE:
	...
	case GROWSPARK:
		...
		sprite[j].pal = 2;
		sprite[j].cstat |= CSTAT_CENTERED|CSTAT_TRANS;
...
unsigned long WeaponToSend = 0;
unsigned long BitsToSend = 0;
bool SendEscape;
...
CVAR (Float,	m_yaw,			0.0625f,CVAR_GLOBALCONFIG|CVAR_ARCHIVE)
CVAR (Float,	m_forward,		1.f,	CVAR_GLOBALCONFIG|CVAR_ARCHIVE)
CVAR (Float,	m_side,			0.125f,	CVAR_GLOBALCONFIG|CVAR_ARCHIVE)
...
void getinput(short snum)
{
	...
	if( (p->gm&MODE_MENU) || (p->gm&MODE_TYPE) || (ud.pause_on && !(BitsToSend & (1<<21))) )
	{
		loc.fvel = vel = 0;
		loc.svel = svel = 0;
		loc.avel = angvel = 0;
	...
	myaimmode = BUTTON(gamefunc_Mouse_Aiming) || freelook;

	// Show message when freelook changes
	if (omyaimstat != freelook)
	{
		omyaimstat = freelook;
		FTA(44+myaimmode,p);
	}
	...
	if( BUTTON(gamefunc_Strafe) || lookstrafe)
		svel = -info.dyaw * m_side;
	else
		angvel = info.dyaw * m_yaw;

	if( myaimmode )
	{
		if(ud.mouseflip)
			horiz -= info.dz * m_pitch;
		else
			horiz += info.dz * m_pitch;
...
// [RH] commands from buttons previously handled in getinput
...
void processinput(short snum)
{
	...
	*/
	...
	if( s->extra <= 0 )
	{
		if(p->dead_flag == 0)
		{
			...
			if(p->scream_voice >= 0)
			{
				stopsound (p->scream_voice, p->i);
				p->scream_voice = -1;
			}

			if( s->pal != 1 && (s->cstat&CSTAT_INVISIBLE) == 0) s->cstat = 0;

			if( ud.multimode > 1 && ( s->pal != 1 || (s->cstat&CSTAT_INVISIBLE) ) )
			{
				if(p->frag_ps != snum)
				{
					ps[p->frag_ps].frag++;
				...
				{
					sprintf((char*)tempbuf,"frag %d killed %d\n",p->frag_ps+1,snum+1);
					sendscore(tempbuf);
					//                    Printf(tempbuf);
				}
	...
	*/
	...
	if( psectlotag == 2)
	{
		...
		if( !ActorIsPlaying (pi, DUKE_UNDERWATER) )
	...
	else if(p->jetpack_on)
	{
		...
		else if(p->jetpack_on == 11 && !ActorIsPlaying (pi, DUKE_JETPACK_IDLE))
		{
			// [RH] This sound should always loop even if it isn't marked as such,
			// so there aren't any gaps in playback.
			xyzsound2(DUKE_JETPACK_IDLE,pi,s->x,s->y,s->z,true);
		}
	...
	if(p->spritebridge == 0)
	{
		...
		if( j == PURPLELAVA || sector[s->sectnum].ceilingpicnum == PURPLELAVA )
		{
			...
			{
				if (!ActorIsPlaying (pi, DUKE_LONGTERM_PAIN))
			...
			case HURTRAIL:
				if( rnd(32) )
				{
					...
					else
					{
						...
						p->pals_time = 32;
						s->extra -= 1+(TRAND&3);
						if (!ActorIsPlaying (pi, SHORT_CIRCUIT))
...
HORIZONLY:
	...
	if(truefdist < PHEIGHT && p->on_ground && psectlotag != 1 && shrunk == 0 && sector[p->cursectnum].lotag == 1)
		if( !ActorIsPlaying (pi, DUKE_ONWATER) )
			spritesound(DUKE_ONWATER,pi);
...
SHOOTINCODE:
	...
	case TRIPBOMB_WEAPON:
		if (!VOLUMEONE && p->ammo_amount[TRIPBOMB_WEAPON] > 0 )
		{
			long sx,sy,sz;
			short sect,hw,hitsp;
			...
			if(j == -1 && hw >= 0 && (wall[hw].cstat&WSTAT_MASKED) == 0 )
	...
	case FREEZE_WEAPON:
		if(!VOLUMEONE && p->ammo_amount[FREEZE_WEAPON] > 0 )
		{
			(*kb) = 1;
	...
	case DEVISTATOR_WEAPON:
		if(!VOLUMEONE && p->ammo_amount[DEVISTATOR_WEAPON] > 0 )
		{
			(*kb) = 1;
