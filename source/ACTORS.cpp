...
void dointerpolations(long smoothratio)       //Stick at beginning of drawscreen
{
	...
	{
		bakipos[i] = *curipos[i];
		delta = (*curipos[i])-oldipos[i];
		j = mulscale16(delta,smoothratio);
...
{
    if( (sector[sectnum].ceilingstat&SSTAT_PARALLAX) && sector[sectnum].ceilingpal == 0 )
    {
...
{
    if( (sector[sectnum].floorstat&SSTAT_PARALLAX) && sector[sectnum].ceilingpal == 0 )
    {
...
void hitradius( short i, long  r, long  hp1, long  hp2, long  hp3, long  hp4 )
{
    ...
    for(x = 0;x<7;x++)
    {
        ...
        while(j >= 0)
        {
            ...
            {
                if( s->picnum != SHRINKSPARK || (sj->cstat&CSTAT_ALLBLOCKING) )
                    if( dist( s, sj ) < r )
                    {
            ...
            else if( sj->extra >= 0 && sj != s && ( sj->picnum == TRIPBOMB || badguy(sj) || sj->picnum == QUEBALL || sj->picnum == STRIPEBALL || (sj->cstat&CSTAT_ALLBLOCKING) || sj->picnum == DUKELYINGDEAD ) )
            {
...
void lotsofmoney(spritetype *s, short n)
{
    ...
    for(i=n;i>0;i--)
    {
        ...
        sprite[j].cstat = TRAND&(CSTAT_XFLIP|CSTAT_YFLIP);
    }
}
...
void ms(short i)
{
    //temp_data[0], [1], and [2] are used for all the sector moving stuff!!!
    ...
    while(i >= 0)
    {
       ...
       //             j = 1;
                    ...
                    if(j) switch(s->picnum)
                    {
                        ...
                        case TRIPBOMB:
                            if (sector[s->sectnum].ceilingstat&SSTAT_PARALLAX)
    ...
    for(q=numcyclers-1;q>=0;q--)
    {
        ...
        if(c[5])
        {
                ...
                if( wal->hitag != 1 )
            {
                ...
                if( (wal->cstat&WSTAT_SWAPBOTTOMS) && wal->nextwall >= 0)
    ...
    while(i >= 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                sprite[i].cstat = CSTAT_INVISIBLE;
            }
        }
...
//                    sprite[j].pal = sprite[i].pal;
            ...
            case MUSICANDSFX:
                ...
                else if(s->lotag < 999 && (unsigned)sector[s->sectnum].lotag < 9 && AmbienceToggle && sector[sprite[i].sectnum].floorz != sector[sprite[i].sectnum].ceilingz)
                {
                    if( (S_sfx[s->lotag].bMusicAndSfx) )
                    {
                        x = dist(&sprite[ps[screenpeek].i],s);
                        if( x < ht && (hittype[i].temp_data[0] == 0 || !ActorIsPlaying(i,s->lotag)) )
                        {
							// [RH] The old code checked if(numenvsnds == NumVoices). Now, I
							// actually try playing the sound and throw out a
							// farther one only if it can't be started.
							int voice;

							// Drop the priority so it doesn't throw out one for us.
							S_sfx[s->lotag].Priority--;

							// These sounds always loop even if they aren't flagged as such.
							voice = xyzsound2(s->lotag,i,s->x,s->y,s->z,true);
							S_sfx[s->lotag].Priority++;

							if (voice > 0)
							{
								FX_SetPriority (voice, S_sfx[s->lotag].Priority);
								hittype[i].temp_data[0] = 1;
							}
							else
                            {
                                ...
                                while(j >= 0)
                                {
                                    ...
                                    {
                                        stopsound(sprite[j].lotag,j);
                                        break;
                                    }
                                    j = nextspritestat[j];
                                }
                                if(j == -1) goto BOLT;
								// [RH] Don't claim this is playing if it couldn't start a voice.
								hittype[i].temp_data[0] = xyzsound2(s->lotag,i,s->x,s->y,s->z,true) > 0;
                            }
                        }
                        if( x >= ht && hittype[i].temp_data[0] == 1 )
                        {
                    ...
                    if(s->extra <= 0)
                    {
                        ...
                        while(j >= 0)
                        {
                            if(sprite[j].hitag == sprite[i].hitag)
                            {
                                ...
                                sprite[j].cstat &= ~CSTAT_ONESIDED;
                                if(sprite[j].picnum == CEILINGSTEAM || sprite[j].picnum == STEAM)
                                    sprite[j].cstat |= CSTAT_INVISIBLE;
                            }
                            j = nextspritestat[j];
                        }
                    }
...
//              IFMOVING;
        ...
        if( AFLAMABLE(s->picnum) )
        {
            if(hittype[i].temp_data[0] == 1)
            {
                ...
                if( (hittype[i].temp_data[1]&3) > 0) goto BOLT;
        ...
        if(s->picnum == TRIPBOMB)
        {
            ...
            else
            {
                ...
                s->extra = 1;
                l = s->ang;
                IFHIT { hittype[i].temp_data[2] = 16; }
                s->extra = x;
                s->ang = l;
            }
        ...
        if( s->picnum == FIREEXT )
        {
            ...
            for(k=0;k<16;k++)
            {
                j = EGS(sprite[i].sectnum,sprite[i].x,sprite[i].y,sprite[i].z-(TRAND%(48<<8)),SCRAP3+(TRAND&3),-8,48,48,TRAND&2047,(TRAND&63)+64,-(TRAND&4095)-(sprite[i].zvel>>2),i,5);
                sprite[j].pal = 2;
            }
        ...
        switch(s->picnum)
        {
            ...
            case SIDEBOLT1+3:
				...
				// [RH] l is used undefined here, and I can't tell what it's supposed to be.
				// I am going to guess based on what I see for the BOLTs below
				l = global_random;

                if(l&1) s->cstat ^= CSTAT_TRANS;
    ...
    while(i >= 0)
    {
        ...
        switch(s->picnum)
        {
            ...
            case TONGUE:
                ...
                if(hittype[i].temp_data[1] > 2047) KILLIT(i);
            ...
            case COOLEXPLOSION1:
                ...
                if( s->picnum == COOLEXPLOSION1 )
					if (!ActorIsPlaying (i, WIERDSHOT_FLY))
                    ...
                    if( (j&49152) == 49152 )
                    {
                        ...
                        if(sprite[j].picnum == APLAYER)
                        {
                            ...
                            if(s->picnum == SPIT)
                            {
                                ...
                                if(ps[p].loogcnt == 0)
                                {
									if (!ActorIsPlaying (ps[p].i, DUKE_LONGTERM_PAIN))
                    ...
                    if(s->picnum != SPIT)
                    {
                        if(s->picnum == RPG)
                        {
                            ...
                            else if( (j&49152) == 16384)
                            {
                                ...
                                else { sprite[k].cstat |= CSTAT_YFLIP; sprite[k].z += (48<<8); }
                            }
                        }
                        ...
                        else if( s->picnum != COOLEXPLOSION1 && s->picnum != FREEZEBLAST && s->picnum != FIRELASER)
                        {
                            ...
                            {
                                if( s->zvel < 0)
                                    { sprite[k].cstat |= CSTAT_YFLIP; sprite[k].z += (72<<8); }
                            }
                        }
                        if( s->picnum == RPG )
                        {
...
void moveactors(void)
{
    ...
    while(i >= 0)
    {
        ...
        switch(s->picnum)
        {
            case DUCK:
            case TARGET:
                if((s->cstat&CSTAT_TYPEMASK) == CSTAT_FLOOR)
                {
                    t[0]++;
                    if(t[0] > 60)
                    {
                        t[0] = 0;
                        s->cstat = CSTAT_CENTERED + CSTAT_ALLBLOCKING + CSTAT_WALL;
                        s->extra = 1;
                    }
                }
                else
                {
                    ...
                    if( j >= 0 )
                    {
                        s->cstat = CSTAT_FLOOR + CSTAT_CENTERED;
                        k = 1;
                        ...
                        while(j >= 0)
                        {
                            ...
                            {
                                if( ( sprite[j].hitag && (sprite[j].cstat&CSTAT_TYPEMASK) != CSTAT_FLOOR) ||
                                    (!sprite[j].hitag && (sprite[j].cstat&CSTAT_TYPEMASK) == CSTAT_FLOOR)
                                  )
                                {
                                    k = 0;
                                    break;
            ...
            case RAT:
                ...
                {
                    hittype[i].temp_data[0]++;
                    if(hittype[i].temp_data[0] > 1) { KILLIT(i); }
            ...
            case STRIPEBALL:
                if(s->xvel)
                {
                    ...
                    {
                        s->cstat = CSTAT_ALLBLOCKING;
                        s->cstat |= CSTAT_XFLIP & s->xvel;
                        s->cstat |= CSTAT_YFLIP & s->xvel;
                    }
                }
                else
                {
...
//                        if(s->pal == 12)
            ...
            case FORCESPHERE:
                ...
                if(s->yvel == 0)
                {
                        ...
                        for(j=0;j<2048;j += 128)
                    {
                        ...
                        sprite[k].cstat = CSTAT_ALLBLOCKING + CSTAT_CENTERED;
            ...
            case RECON:
                ...
                if( ud.multimode < 2 )
                {
                    ...
                    else if(actor_tog == 2) s->cstat = CSTAT_ALLBLOCKING;
                }
                IFHIT
                {
                    if( s->extra < 0 && t[0] != -1 )
...
// #endif
                ...
                if(sector[sect].floorstat&SSTAT_PARALLAX)
                    KILLIT(i);
                ...
                if(t[0] == -5) // FROZEN
                {
                    ...
                    IFHIT
                    {
                        ...
                        for(j=16; j >= 0 ;j--)
                        {
                            k = EGS(sprite[i].sectnum,sprite[i].x,sprite[i].y,sprite[i].z,GLASSPIECES+(j%3),-32,36,36,TRAND&2047,32+(TRAND&63),1024-(TRAND&1023),i,5);
                            sprite[k].pal = 1;
                        }
				...
				s->cstat = (x < 1596) ? 0 : CSTAT_ALLBLOCKING;
  ...
  //                      guts(s,JIBS6,5,myconnectindex);
                ...
                if(t[0] == 0 || t[0] == 2)
                {
                    ...
                    if(t[0]==2)
                    {
                        ...
                        if( (sector[sect].ceilingstat&SSTAT_PARALLAX) || (hittype[i].ceilingz+6144) < s->z)
                        {
                            s->z += 2048;
                            t[0] = 3;
                            goto BOLT;
...
// TJR
                    ...
                    if(rnd(4) && (sector[sect].ceilingstat&SSTAT_PARALLAX) == 0 &&
            ...
            case HEAVYHBOMB:
                ...
                if( x < 1220 ) s->cstat &= ~CSTAT_ALLBLOCKING;
                else s->cstat |= CSTAT_ALLBLOCKING;
                ...
                if(s->xvel > 0)
                {
                    ...
                    if(s->xvel < 0)
                        s->xvel = 0;
                    if(s->xvel&8) s->cstat ^= CSTAT_XFLIP;
                }
            ...
            case WATERSPLASH2:
                ...
                if(t[0] == 1 )
                {
                    if(sector[sect].lotag != 1 && sector[sect].lotag != 2)
                        KILLIT(i);
/*                    else		[RH] commented out in original source
                    {
                        l = getflorzofslope(sect,s->x,s->y)-s->z;
                        if( l > (16<<8) ) KILLIT(i);
                    }
                    else */ if(!ActorIsPlaying (i, ITEM_SPLASH))
            ...
            case FRAMEEFFECT1:
                ...
                if(s->owner >= 0)
                {
                    ...
                    if( t[0] > 7 )
                    {
                        ...
                        s->cstat |= CSTAT_REVTRANS + CSTAT_TRANS;
                    else if( t[0] > 2 )
                        s->cstat |= CSTAT_TRANS;
            ...
            case PAPER:
                ...
                s->xvel = (TRAND&7)+(sintable[hittype[i].temp_data[0]&2047]>>9);
                hittype[i].temp_data[0] += (TRAND&63);
            ...
            case DUKELEG:
                ...
                else
                {
                    ...
                    {
                        if( s->sectnum == -1) { KILLIT(i); }
                        if( (sector[s->sectnum].floorstat&SSTAT_GROUDRAW) ) { KILLIT(i); }
                        t[2]++;
                    }
                    l = getflorzofslope(s->sectnum,s->x,s->y);
            ...
            case PUKE:
                ...
                {
                    t[0] = 1;
                    if(sector[sect].floorstat&SSTAT_GROUDRAW) { KILLIT(i); }
    ...
    while(i >= 0)
    {
        ...
        switch(st)
        {
            case 0:
            {
                ...
                if( l && (sc->floorstat&SSTAT_ALIGNTOWALL) )
                {
...
//                        if( t[4] == 1 )
            ...
            case 14:
                ...
                if(s->xvel)
                {
                    ...
                    if(s->xvel == sc->extra )
                    {
                        if( (sc->floorstat&SSTAT_PARALLAX) == 0 && (sc->ceilingstat&SSTAT_PARALLAX) == 0 )
                        {
                            if( !ActorIsPlaying (i, hittype[i].lastvx))
                                spritesound(hittype[i].lastvx,i);
                        }
                        else if( ud.monsters_off == 0 && sc->floorpal == 0 && (sc->floorstat&SSTAT_PARALLAX) && rnd(8) )
                        {
                            p = findplayer(s,&x);
                            if(x < 20480)
                            {
                    ...
                    if(s->xvel <= 64 && (sc->floorstat&SSTAT_PARALLAX) == 0 && (sc->ceilingstat&SSTAT_PARALLAX) == 0 )
            ...
            case 4:
                ...
                while(j >= 0)
                {
                    if((sprite[j].cstat&CSTAT_TYPEMASK) == CSTAT_WALL)
                    {
                        if (sc->ceilingstat&SSTAT_PARALLAX)
                ...
                if(t[0])
                {
                    if(t[0] == 1)
                    {
                        ...
                        for(j=x;j<q;j++)
                            ...
                            {
                                wall[j].cstat &= WSTAT_TRANS + WSTAT_ONEWAY + WSTAT_XFLIP + WSTAT_ALIGNBOTTOM + WSTAT_SWAPBOTTOMS;
                                wall[j].overpicnum = 0;
                                if(wall[j].nextwall >= 0)
                                {
                                    wall[wall[j].nextwall].overpicnum = 0;
                                    wall[wall[j].nextwall].cstat &= (WSTAT_TRANS + WSTAT_ONEWAY + WSTAT_XFLIP + WSTAT_ALIGNBOTTOM + WSTAT_SWAPBOTTOMS);
                                }
                            }
                    }
...
//                                case 18:
                ...
                if( s->xvel ) //Moving
                {
                    ...
                    while(j >= 0)
                    {
                        ...
                        if( sprite[j].statnum != 3 && sprite[j].zvel == 0)
                        {
                            ...
                            sprite[j].y += l;
                            setsprite(j,sprite[j].x,sprite[j].y,sprite[j].z);
                            if( sector[sprite[j].sectnum].floorstat&SSTAT_GROUDRAW )
            ...
            case 28:
                ...
                if(hittype[i].temp_data[0] == 0)
                {
                    ...
                    if( x > 15500 )
                        break;
                    hittype[i].temp_data[0] = 1;
                    hittype[i].temp_data[1] = 64 + (TRAND&511);
                    hittype[i].temp_data[2] = 0;
                }
                else
                {
                    ...
                    else if(hittype[i].temp_data[2] > (hittype[i].temp_data[1]>>3) && hittype[i].temp_data[2] < (hittype[i].temp_data[1]>>2) )
                    {
                        ...
                        while(j >= 0)
                        {
                            if( sprite[j].picnum == NATURALLIGHTNING && sprite[j].hitag == s->hitag)
                            {
                                ...
                                else sprite[j].cstat |= CSTAT_INVISIBLE;
                            }
           ...
           case 32: // True Drop Ceiling
            ...
            case 128: //SE to control glass breakage
				...
				// [RH] This if will always be true. Should the '|' be a '&'?
                if(wal->cstat|WSTAT_ONEWAY)
                {
                    wal->cstat &= ~(WSTAT_YFLIP|WSTAT_REVTRANS|WSTAT_ONEWAY);
                    wal->cstat |= WSTAT_MASKED;
                    if(wal->nextwall >= 0)
                    {
                        wall[wal->nextwall].cstat &= ~(WSTAT_YFLIP|WSTAT_REVTRANS|WSTAT_ONEWAY);
                        wall[wal->nextwall].cstat |= WSTAT_MASKED;
                    }
                }
                else break;
                ...
                {
                    wal->cstat &= WSTAT_TRANS + WSTAT_ONEWAY + WSTAT_XFLIP + WSTAT_ALIGNBOTTOM + WSTAT_SWAPBOTTOMS;
                    if(wal->nextwall >= 0)
                        wall[wal->nextwall].cstat &= WSTAT_TRANS + WSTAT_ONEWAY + WSTAT_XFLIP + WSTAT_ALIGNBOTTOM + WSTAT_SWAPBOTTOMS;
                    KILLIT(i);
                }
                break;
