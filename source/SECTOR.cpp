    ...
    while(i >= 0)
    {
        if( sprite[i].picnum == MUSICANDSFX && sprite[i].lotag < 1000 )
        {
            ...
            else if(sprite[i].hitag < NUM_SOUNDS)
            {
                ...
                if( (S_sfx[sprite[i].lotag].bRepeat) || ( sprite[i].hitag && sprite[i].hitag != sprite[i].lotag ) )
                    stopsound(sprite[i].lotag,whatsprite);
...
char checkhitswitch(short snum,long w,char switchtype)
{
    ...
    switch(picnum)
    {
        ...
        case PULLSWITCH+1:
                ...
                {
                    if(switchtype == 1 && (S_sfx[hitag].bDukeVoice) == 0)
   ...
   if( ( (wal->cstat&16) || wal->overpicnum == BIGFORCE ) && wal->nextsector >= 0 )
               ...
               switch(wal->overpicnum)
    {
        ...
        case BIGFORCE:
            ...
            sprite[i].cstat |= CSTAT_WALL+CSTAT_TRANS+CSTAT_CENTERED;
    ...
    switch(sprite[i].picnum)
    {
        ...
        case STRIPEBALL:
            if(sprite[sn].picnum == QUEBALL || sprite[sn].picnum == STRIPEBALL)
            {
                ...
                if (!ActorIsPlaying (i, POOLBALLHIT))
...
//        case CACTUSBROKE:
        ...
        case GENERICPOLE2:
            for(k=0;k<6;k++)
                EGS(sprite[i].sectnum,sprite[i].x,sprite[i].y,sprite[i].z-(8<<8),SCRAP1+(TRAND&15),-8,48,48,TRAND&2047,(TRAND&63)+64,-(TRAND&4095)-(sprite[i].zvel>>2),i,5);
        ...
        case BROKEHYDROPLANT:
            if(sprite[i].cstat&CSTAT_CLIPBLOCKING)
            {
                spritesound(GLASS_BREAKING,i);
                sprite[i].z += 16<<8;
                sprite[i].cstat = 0;
        ...
        case TOILET:
            ...
            sprite[i].cstat |= (TRAND&1) << CSTATB_XFLIP;
            sprite[i].cstat &= ~CSTAT_ALLBLOCKING;
            spawn(i,TOILETWATER);
            spritesound(GLASS_BREAKING,i);
            break;
        ...
        default:
            ...
            if( ( sprite[sn].picnum == FREEZEBLAST || sprite[sn].owner != i ) && sprite[i].statnum != 4)
            {
                if( badguy(&sprite[i]) == 1)
                {
                    ...
                    if( sprite[i].picnum != TANK &&
						sprite[i].picnum != BOSS1 &&
						sprite[i].picnum != BOSS4 &&
						sprite[i].picnum != BOSS2 &&
						sprite[i].picnum != BOSS3 &&
						sprite[i].picnum != RECON &&
						sprite[i].picnum != ROTATEGUN )
                    {
...
void checksectors(short snum)
{
    ...
    else if(!p->toggle_key_flag)
    {
        ...
        if(i < 1280 && hitscanwall >= 0 && wall[hitscanwall].overpicnum == MIRROR)
            if( wall[hitscanwall].lotag > 0 && snum == screenpeek && !ActorIsPlaying (p->i, wall[hitscanwall].lotag))
        {
        ...
        if(neartagsprite >= 0)
        {
            ...
            switch(sprite[neartagsprite].picnum)
            {
                ...
                case STALL:
                    ...
                    else if(!ActorIsPlaying (p->i, FLUSH_TOILET))
